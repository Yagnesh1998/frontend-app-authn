...
import {
  FIELDS, FORBIDDEN_REQUEST, INTERNAL_SERVER_ERROR,
} from '../data/constants';
import EmbedableRegistrationForm from '../EmbedableRegistrationForm';
...
const IntlRegistrationPage = injectIntl(EmbedableRegistrationForm);
...
describe('RegistrationPage', () => {
  ...
  describe('Test Registration Page', () => {
    ...
    // //   expect(registrationPage.find('div[feedback-for="country"]').exists()).toBeTruthy();
    // // });
    ...
    it('should give email suggestions for common service provider domain typos', async () => {
      store.dispatch = jest.fn(store.dispatch);
      const registrationPage = mount(reduxWrapper(<IntlRegistrationPage {...props} />));

      registrationPage.find('input#email').simulate('focus');
      registrationPage.find('input#email').simulate('change', { target: { value: 'johns@yopmail.com', name: 'email' } });
      registrationPage.find('input#email').simulate('blur');
      registrationPage.find('button.btn-brand').simulate('click');
      expect(registrationPage.find('#email-warning').text()).toEqual('Did you mean: johns@hotmail.com?');
    });
    ...
    it('should give error for common top level domain mistakes', () => {
      ...
      expect(registrationPage.find('.email-suggestion__text').text()).toEqual('Did you mean john@gmail.com?');
    });
    ...
    it('should update props with validations returned by registration api', () => {
      ...
      const registrationPage = mount(reduxWrapper(<IntlRegistrationPage {...props} />)).find('EmbedableRegistrationForm');
    ...
    //   registrationPage.find('input#password').simulate('change',
    // { target: { value: 'aziz194@', name: 'password' } });
  ...
  describe('Test Configurable Fields', () => {
    ...
    it('should show error if email and confirm email fields do not match', () => {
      ...
      registrationPage.find('input#confirm_email').simulate('change', { target: { value: 'test2@gmail.com', name: 'confirm_email' } });
      registrationPage.find('input#confirm_email').simulate('focus');
      registrationPage.find('button.btn-brand').simulate('click');
